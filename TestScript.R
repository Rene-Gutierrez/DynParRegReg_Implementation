# This file Creates the data and runs one instance of DFP with the Bayesian
# Lasso prior for the dense setting discussed in Guhaniyogi and Gutierrez. It
# calls a file dense_data.R that generates the data. It uses the functions
# described in the DynParRegReg package available in github. To install the
# package please run the following:
# devtools::install_github("Rene-Gutierrez/DynParRegReg")

### Creates the Data
source('./dense_data.R')

### Creates a Data Directory (If necessary)
if(!dir.exists("./out")){
  dir.create("./out")
}

# Problem Type
datTyp    <- "complete"
regMod    <- "bayLas"
met       <- "dfp"

# Set-Up
ite    <- 1
per    <- get(load(file = paste0('./dat/per-', ite,'.RData')))
coe    <- get(load(file = paste0('./dat/coe-',ite,'.RData')))
gr1    <- get(load(file = paste0('./dat/gr1-',ite,'.RData')))
rb     <- get(load(file = './dat/rb.RData'))
M      <- 6
# Initial Random Partition
p      <- length(coe)
P      <- list()
rem    <- p %% M
if(rem == 0){
  comNum <- p %/% M
} else {
  comNum <- p %/% M + 1
}
for(i in 1:(comNum - 1)){
  P[[i]] = coe[((i - 1) * M + 1):(i * M)]
}
if(rem == 0){
  P[[comNum]] <- coe[((comNum - 1) * M + 1):(comNum * M)]
} else {
  P[[comNum]] <- coe[((comNum - 1) * M + 1):((comNum - 1) * M + rem)]
}

# Creates the Paths
curParPat <- paste0('./out/curPar-', datTyp, "-", regMod, "-", met, "-", ite, "-", M, ".RData")
savPat    <- paste0('./out/savPar-', datTyp, "-", regMod, "-", met, "-", ite, "-", M, ".RData")
staPat    <- paste0('./out/sta-', datTyp, "-", regMod, "-", met, "-", ite, "-", M, ".RData")

out <- dfpBayLas(staBat    = 1,
                 endBat    = 500,
                 savCoe    = gr1,
                 curParPat = curParPat,
                 savPat    = savPat,
                 staPat    = staPat,
                 datPer    = per[ite],
                 M         = M,
                 iniPar    = P,
                 staPar    = list(hb = rep(0, p),
                                  ht = rep(1, p),
                                  hs = 1,
                                  hl = 1))
